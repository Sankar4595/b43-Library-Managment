...
const EditHistory = ({history,setHistory,newItem}) => {
  const {id} = useParams();

  const selectBook = history.find((data)=>data.id ===id);
  ...
  const updatebook = (val) => {
    const editedIndex = history.findIndex((data) => data.id === id);
    history[editedIndex] = val;
    setHistory([...history]);
    newItem.push("/history");
  };
