...
const EditTamil = ({tamil,setTamil,newItem}) => {
  const {id} = useParams();

  const selectBook = tamil.find((data)=>data.id ===id);
  ...
  const updatebook = (val) => {
    const editedIndex = tamil.findIndex((data) => data.id === id);
    tamil[editedIndex] = val;
    setTamil([...tamil]);
    newItem.push("/tamil");
  };
